...
independently, and measured 11{,}136 query executions against
PostgreSQL 17.1 across two table scales, eleven dataset configurations,
ten index configurations and seven cost-model settings. Ground-truth
...
though only while the table is resident in the buffer pool. Each
...
\textbf{Boundary.} The effect does not survive scale. At ten million
rows the two configurations are statistically indistinguishable. The
\texttt{phys} datasets, which vary the physical correlation BRIN
depends on, confirm this directly: \texttt{phys05} moves from 90.0\%
versus 0.0\% at 1M to 50.0\% versus 57.1\% at 10M, with the BRIN
configuration marginally better. \Cref{fig:brincorr} shows how each
index type responds to physical correlation.
...
The two scales are genuinely different regimes, confirmed independently
by the buffer statistics: blocks read from outside the buffer pool are
\textbf{0 at one million rows and 89{,}081 at ten million}. Why the
penalty disappears is \emph{not} established. BRIN is selected about
equally often at both scales, roughly 4.5\% of range queries, so it is
not that the planner stops choosing it. We record this as an open
question rather than offer an unverified explanation.
...
194 times, but only while the table is buffer-pool resident.

That last boundary is as much a part of the result as the effect
itself. A tuning recommendation is not right or wrong in general; it is
right or wrong under conditions, and those conditions are measurable.

\textbf{Future work.} Four directions would strengthen these findings:
confirming the path-suppression mechanism against PostgreSQL's source;
locating the precise scale at which the BRIN effect breaks down, which
would convert an open question into a measured boundary tied to buffer
pool size; repeating the study on a second DBMS to test whether the
findings are PostgreSQL-specific; and extending to cold-cache
conditions on hardware where page cache eviction is controllable.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lp{9cm}@{}}
        ...
        \midrule
        Imtiaj Sajin & Experimental design; benchmark implementation;
